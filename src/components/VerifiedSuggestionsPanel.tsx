      ...
      <div className="flex flex-wrap items-start gap-2 px-3 py-2.5">
        ...
        >
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="rounded-md border border-emerald-400/35 bg-emerald-500/15 px-1.5 py-0.5 text-[12px] font-semibold text-emerald-100">
              מאגר מאומת
            </span>
            {sug.matchScore != null ? (
              <span className="text-[14px] text-ink-dim">ציון {sug.matchScore}</span>
            ) : null}
          </div>
      ...
      {expanded ? (
        <div className="border-t border-white/10 px-3 pb-3 pt-2 space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              className="min-h-[40px] flex-1 rounded-xl bg-white px-3 py-2 text-sm font-semibold text-black"
            ...
            <button
              type="button"
              className="min-h-[40px] rounded-xl border border-white/20 px-3 py-2 text-sm font-semibold text-white hover:border-white/35"
        ...
        <div className="flex flex-wrap gap-1.5 border-t border-white/5 px-3 pb-2 pt-1">
          <button
            type="button"
            className="rounded-lg bg-white/10 px-2 py-1 text-sm font-semibold text-white hover:bg-white/15"
            onClick={() => onPickNutrition(sug)}
          >
            מלא תזונה
          </button>
...
export function VerifiedSuggestionsPanel({
  ...
  if (!verifiedSearchQueryReady(searchQuery) && !verifiedPicked) {
    return (
      <p className="rounded-xl border border-emerald-400/20 bg-emerald-500/[0.04] px-3 py-2 text-[14px] text-ink-dim">
    ...
    <div className="space-y-3 rounded-2xl border border-emerald-400/20 bg-emerald-500/[0.04] p-3">
      ...
      {verifiedPicked ? (
        <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-emerald-400/25 bg-emerald-500/10 px-3 py-2">
          <span className="text-[14px] font-semibold text-emerald-50">✓ נבחרה תזונה מהמאגר המאומת</span>
          {!isAlreadyInCatalog ? (
            <button
              type="button"
